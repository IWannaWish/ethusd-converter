...
const module = "internal/eth/source/asset_service.go"
...
func (s *ethAssetService) GetAssets(ctx context.Context, address common.Address) ([]core.Asset, error) {
	...
	for _, src := range s.sources {
		...
		price, ok := src.Feed.Get(ctx, symbol)
		if !ok {
			s.logger.Error(ctx, "не удалось получить цену", applog.String("symbol", symbol), applog.String("module", module))
			continue
		}
